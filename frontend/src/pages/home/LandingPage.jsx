...
import featuresBackground from '../../assets/images/tripexplorebg.png';
...
const LandingPage = () => {
  ...
  return (
    <div style={{ position: 'relative', minHeight: '2800px', backgroundColor: '#0D2E3F' }}>
      ...
      <Navbar variant="landing" style={{ position: 'sticky', top: 0, left: 0, right: 0, zIndex: 60, backgroundColor: `${colors.bg}33`, backdropFilter: 'saturate(120%) blur(6px)', borderBottom: `1px solid ${colors.bg}20` }} />
      ...
      <main style={{ padding: 32, height: '1832px', position: 'relative', zIndex: 10, display: 'flex', alignItems: 'flex-start' }}>
        ...
        <section style={{ position: 'absolute', left: 0, right: 0, top: 500, padding: '0 32px', zIndex: 10 }}>
          <div style={{ marginLeft: 98, display: 'flex', width: 564, flexDirection: 'column', alignItems: 'flex-start', gap: 25 }}>
            <div style={{ display: 'flex', height: 135, flexDirection: 'column', justifyContent: 'center', alignSelf: 'stretch', color: '#FADD9B', fontFamily: 'Poppins, sans-serif', fontSize: 64, fontStyle: 'normal', fontWeight: 700, lineHeight: '70px' }}>
              From Origin to The Chosen Way
            </div>

            <div style={{ alignSelf: 'stretch', color: '#FFFFFF', fontFamily: 'Lora, serif', fontSize: 20, fontStyle: 'normal', fontWeight: 700, lineHeight: 'normal' }}>
              Orivia is a travel platform that helps users move from their starting point to their chosen path.
            </div>

            <div style={{ display: 'flex', padding: 0, justifyContent: 'flex-start', alignItems: 'center', gap: 5 }}>
              <Button variant="btn2" onClick={() => navigate('/login')}>
        ...
        <section style={{ position: 'absolute', left: 0, right: 0, top: 1200, padding: '0 32px', display: 'flex', justifyContent: 'center', zIndex: 10 }}>
          <div style={{ width: 1600, maxWidth: '100%', display: 'flex', gap: 64, alignItems: 'stretch' }}>
            ...
            <div style={{ width: '52%', borderRadius: 28, padding: 56, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', background: 'linear-gradient(165deg, #0A3040 0%, #0E5058 100%)', color: '#fff', boxShadow: '0 12px 32px rgba(0,0,0,0.25)' }}>
              <div>
                <h2 style={{ margin: 0, color: '#FADD9B', fontFamily: 'Poppins, sans-serif', fontSize: 52, fontWeight: 800, lineHeight: '1.08', letterSpacing: '-0.02em' }}>
                  We Are The Best Way To Plan Or Follow Your Journey
                </h2>
                <p style={{ marginTop: 24, marginBottom: 40, color: '#ffffff', fontFamily: 'Lora, serif', fontSize: 20, fontWeight: 400, lineHeight: 1.6, opacity: 0.95 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
                {[
                  ...
                  <div key={index} style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
                    <div style={{ width: 72, height: 72, borderRadius: 36, background: '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 8px 20px rgba(0,0,0,0.18)', flexShrink: 0 }}>
                      <FontAwesomeIcon icon={feature.icon} style={{ fontSize: 32, color: '#0D2E3F' }} />
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ color: '#FADD9B', fontWeight: 700, fontSize: 19, fontFamily: 'Poppins, sans-serif', marginBottom: 8 }}>{feature.title}</div>
                      <div style={{ color: '#fff', fontFamily: 'Lora, serif', fontSize: 17, lineHeight: 1.5, opacity: 0.9 }}>{feature.description}</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <section style={{ position: 'absolute', left: 0, right: 0, top: 2000, zIndex: 10 }}>
          ...
          <div style={{ position: 'relative', width: '100%', minHeight: 480, padding: '48px 130px', display: 'flex', flexDirection: 'column', gap: 40 }}>
            {/* Header */}
            <div>
                <h2 style={{ margin: 0, color: '#FADD9B', fontFamily: 'Poppins, sans-serif', fontSize: 48, fontWeight: 800, lineHeight: 1.2, marginBottom: 16 }}>
                  What We Have
                </h2>
